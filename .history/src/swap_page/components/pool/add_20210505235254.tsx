...
import { CREATE_POOL_LABEL, ADD_LIQUIDITY_LABEL, generateActionLabel, generateExactOneLabel } from '../labels'
...
export const AddToLiquidity = () => {
  ...
  const executeAction = !connected
    ...
    : async (instance?: PoolInfo) => {
        ...
        if (isLatestLayout && depositType === 'one' && currentDepositToken?.account && currentDepositToken.mint) {
          setPendingTx(true)
          const components = [
            {
  ...
  const createPoolButton = pool && (
    <Button
      ...
      style={{ width: '100%', backgroundColor: 'yellow', borderColor: 'black', color: 'black' }}
      size="large"
      onClick={() => executeAction()}
      disabled={
        connected && (pendingTx || !A.account || !B.account || A.account === B.account || !hasSufficientBalance)
      }
    >
  ...
  const addLiquidityButton = (
    <Dropdown.Button
      ...
      trigger={['click']}
      style={{ width: '100%', backgroundColor: 'yellow', color: 'black' }}
      disabled={
        connected &&
        (depositType === 'both'
          ? pendingTx || !A.account || !B.account || A.account === B.account || !hasSufficientBalance
          : !getDepositToken()?.account || !getDepositToken()?.sufficientBalance())
      }
      size="large"
  ...
  const getTokenOptions = () => {
    ...
    return (
      <>
        ...
        {[A, B].map((item) => {
          return (
            <Radio key={item.mintAddress} value={item.mintAddress} name={item.name}>
              Add {item.name}
            </Radio>
          )
        })}
        ...
        {isLatestLayout && pool && (
          <div className="flex-row-center">
            <Radio.Group
              ...
              value={depositType === 'both' ? pool?.pubkeys.mint.toBase58() : getDepositToken()?.mintAddress}
            >
              {getTokenOptions()}
            </Radio.Group>
          </div>
    ...
    >
      <div className="pool-card" style={{ width: 'initial' }}>
        ...
        <div className="pool-card-row" style={{ margin: 0 }}>
          ...
          <div className="pool-card-cell">{formatPriceNumber.format(ratio * enriched.liquidityA)}</div>
        </div>
        <div className="pool-card-row" style={{ margin: 0 }}>
          <div className="pool-card-cell">{enriched.names[1]}:</div>
          <div className="pool-card-cell">{formatPriceNumber.format(ratio * enriched.liquidityB)}</div>
        </div>
      </div>
    </Card>
  )
...
export const AddToLiquidityView = () => {
  return (
    <>
      ...
      />
      <Card className="exchange-card" headStyle={{ padding: 0 }} bodyStyle={{ position: 'relative' }}>
        <AddToLiquidity />
      </Card>
    </>
  )
