...
export default function RewardsStakeBottomBar({
  ...
  return (
    <div
      css={[
        tw`mt-3 min-md:mt-auto flex min-md:max-w-[324px] h-17.25 min-md:p-2.5 min-md:bg-gradient-to-r from-green-gradient-3
    to-green-gradient-4 bg-none rounded-t-tiny text-average min-md:text-tiny font-semibold text-grey-1
     dark:text-grey-2  min-md:text-grey-5
        min-md:dark:text-grey-5 items-center flex-col min-md:flex-row w-full`
      ]}
    >
      <div css={[tw`flex min-md:flex-col min-md:gap-1.25 justify-between min-md:justify-start w-full`]}>
        <p css={[tw`mb-0`]}>Approx. Daily Rewards</p>
        <p css={[tw` mb-0 text-black-4 dark:text-grey-5`]}>
          {calculating ? (
            <Skeleton height={'15px'} width={'60px'} borderRadius={'1rem'} />
          ) : (
