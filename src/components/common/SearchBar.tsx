...
const SearchBar = ({
  ...
  return (
    <InputGroup
      ...
      rightItem={
        showRight ? (
          <InputElementRight className={'flex gap-1.25'} onClick={onClear} show={showRight}>
            {additionalInputElementRight}
            {isLoading && <Icon src={`/img/assets/refresh_${mode}.svg`} className={'animate-spin'} size={'sm'} />}
            <Icon
              size={'sm'}
          ...
          <InputElementRight className={'flex gap-1.25'} onClick={onClear}>
            {isLoading && <Icon src={`/img/assets/refresh_${mode}.svg`} className={'animate-spin'} size={'sm'} />}
          </InputElementRight>
        )
      }
