...
// change stake prefix
export const getSslAccountKey = async (wallet: WalletContextState): Promise<undefined | PublicKey> => {
  try {
...
export const createLiquidityAccountIX = async (program: Program<Idl>) => {
  const createLiquidityInstructionAccounts = {
    controller: CONTROLLER_KEY,
    ssl: getSslAccountKey,
    liquidityAccount: temp,
    userWallet: temp,
    SystemProgram: temp,
    rent: temp
  }
  const createLiquidityIX: TransactionInstruction = await program.instruction.createLiquidityAccount({
    accounts: createLiquidityInstructionAccounts
  })
