...
const getLikelyPositionMintsForUser = async (connection: Connection, user: PublicKey): Promise<PublicKey[]> => {
...
const getOrcaPositionAccountsForMints = async (
  ...
  const positions = Array.from((await whirlpoolCtx.fetcher.getPositions(addresses, IGNORE_CACHE)).values()).filter(
