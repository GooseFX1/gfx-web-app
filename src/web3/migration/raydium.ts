...
const getCLMMPoolInfosFromAPI = async (
  ...
  await Promise.all(
    chunks(poolIds, 100).map(
      async (ids) => (await raydium.api.fetchPoolById({ ids: ids.join(',') })) as ApiV3PoolInfoConcentratedItem[]
    )
...
const getCLMMPoolInfosFromRPC = async (
  ...
  const rpcPools = await clmm.getRpcClmmPoolInfos({
    poolIds,
    config: {
      batchRequest: true,
      chunkCount: 99
...
export const getRaydiumCLMMPositions = async (
  ...
  const poolsToFetch = ownerPositions
    .map((position) => position.poolId.toString())
    .filter((poolId) => !poolMap[poolId])

  let effectiveEpochInfo = epochInfo;

  if (!effectiveEpochInfo) {
    effectiveEpochInfo = await raydium.connection.getEpochInfo();
  }
