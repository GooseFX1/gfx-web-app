...
export const Orders: FC = () => {
  ...
  const content = useMemo(() => orders.map((order, index) => (
    <Entry key={index} $entriesLength={PANELS_FIELDS[HistoryPanel.Orders].length}>
      <div>
        <span onClick={() => handleClick(order.name)}>
          {formatPair(order.name)}
        </span>
      </div>
      ...
      <div>
        <MainButton height="30px" onClick={() => cancelOrder(order)} status="action" width="150px">
          <span>Cancel order</span>
        </MainButton>
      </div>
    </Entry>
  )), [cancelOrder, formatPair, handleClick, orders])
