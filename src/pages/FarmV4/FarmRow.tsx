...
const FarmRow: FC<FarmRowProps> = ({ pool, ...props }) => {
  ...
  return (
    <div
      className={cn(
        activeReward &&
          'w-full border-transparent bg-gradient-to-r from-[#F7931A] to-[#C31AE3] p-[1px] rounded-tiny'
      )}
    >
      <div
        className={cn(
      ...
      >
        ...
        {(isTablet || isDesktop) && (
          <div className="flex items-center justify-center">
            <Button
              className={cn(
                `cursor-pointer bg-blue-1 text-white h-[30px]`,
                pool.hasDeposit && 'w-[30px] h-[30px]'
              )}
              variant={'secondary'}
              onClick={() => {
                setSelectedCard(pool)
