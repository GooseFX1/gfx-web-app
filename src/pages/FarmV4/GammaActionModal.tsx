      ...
      <DialogPortal>
        ...
        >
          <DialogHeader className={'flex-row justify-between pb-1.25'}>
            <h3 className={'text-start text-text-lightmode-primary dark:text-text-darkmode-primary'}>{title}</h3>
            <DialogClose className={'ml-auto'}>
              <Icon className={'min-w-4 min-h-4 max-w-4 max-h-4'} src={`/img/assets/close-${mode}.svg`} />
            </DialogClose>
          </DialogHeader>
          <DialogBody>{children}</DialogBody>
          <DialogFooter className={'flex-col items-center gap-2.5'}>
            <Button fullWidth colorScheme={'blue'} onClick={onActionClick} isLoading={loading} disabled={loading}>
              {actionLabel}
            </Button>
            <p className={`text-b3 text-text-darkmode-secondary`}>
