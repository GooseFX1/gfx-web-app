...
export class EventQueueHeader {
  ...
  constructor(arg: { tag: number; head: BN; count: BN; eventSize: BN; seqNum: BN }) {
...
export class EventFill {
  ...
  static deserialize(callbackInfoLen: number, data: Buffer) {
    return new EventFill({
      ...
      takerCallbackInfo: [...data.slice(34 + callbackInfoLen, 34 + 2 * callbackInfoLen)]
    })
  }
}
...
export class EventOut {
  ...
  constructor(arg: { side: number; orderId: BN; baseSize: BN; delete: number; callBackInfo: number[] }) {
...
export class EventQueue {
  ...
  constructor(arg: { header: EventQueueHeader; buffer: number[]; callBackInfoLen: number }) {
  ...
  static parse(callBackInfoLen: number, data: Buffer) {
    return new EventQueue({
      header: deserializeUnchecked(EventQueueHeader.schema, EventQueueHeader, data) as EventQueueHeader,
      buffer: [...data],
      callBackInfoLen
    })
  }
   ...
   * @returns Returns an EventQueue object
   */
  static async load(connection: Connection, address: PublicKey, callBackInfoLen: number) {
  ...
  parseEvent(idx: number) {
    const header_offset = EventQueueHeader.LEN + EventQueueHeader.REGISTER_SIZE
    const offset =
      header_offset +
      ((idx * this.header.eventSize.toNumber() + this.header.head.toNumber()) %
        (this.buffer.length - header_offset))
    const data = Buffer.from(this.buffer.slice(offset, offset + this.header.eventSize.toNumber()))
    switch (data[0]) {
      case EventType.Fill:
  ...
  parseFill(limit?: number) {
    const n = limit ? Math.min(limit, this.header.count.toNumber()) : this.header.count.toNumber()
    return [...Array(n).keys()].map((e) => this.parseEvent(e)).filter((e) => e instanceof EventFill)
  }
  ...
  static parseEventQueueHeader(data: Buffer) {
    return deserialize(EventQueueHeader.schema, EventQueueHeader, data) as EventQueueHeader
  }
