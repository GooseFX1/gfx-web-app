import { PublicKey } from "@solana/web3.js" // eslint-disable-line @typescript-eslint/no-unused-vars
import BN from "bn.js" // eslint-disable-line @typescript-eslint/no-unused-vars
import * as types from "../types" // eslint-disable-line @typescript-eslint/no-unused-vars
import * as borsh from "@project-serum/borsh"

export interface WithdrawFundsParamsFields {
  quantity: types.FractionalFields
}

export interface WithdrawFundsParamsJSON {
  quantity: types.FractionalJSON
}

export class WithdrawFundsParams {
  readonly quantity: types.Fractional

  constructor(fields: WithdrawFundsParamsFields) {
    this.quantity = new types.Fractional({ ...fields.quantity })
  }

  static layout(property?: string) {
    return borsh.struct([types.Fractional.layout("quantity")], property)
  }

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  static fromDecoded(obj: any) {
    return new WithdrawFundsParams({
      quantity: types.Fractional.fromDecoded(obj.quantity),
    })
  }

  static toEncodable(fields: WithdrawFundsParamsFields) {
    return {
      quantity: types.Fractional.toEncodable(fields.quantity),
    }
  }

  toJSON(): WithdrawFundsParamsJSON {
    return {
      quantity: this.quantity.toJSON(),
    }
  }

  static fromJSON(obj: WithdrawFundsParamsJSON): WithdrawFundsParams {
    return new WithdrawFundsParams({
      quantity: types.Fractional.fromJSON(obj.quantity),
    })
  }

  toEncodable() {
    return WithdrawFundsParams.toEncodable(this)
  }
}
