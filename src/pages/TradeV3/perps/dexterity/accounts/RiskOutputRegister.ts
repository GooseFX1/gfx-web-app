...
export class RiskOutputRegister {
  ...
  static readonly discriminator = Buffer.from([39, 194, 173, 31, 222, 198, 229, 124])

  static readonly layout = borsh.struct([types.HealthResult.layout('riskEngineOutput')])

  constructor(fields: RiskOutputRegisterFields) {
    this.riskEngineOutput = fields.riskEngineOutput
  }

  static async fetch(c: Connection, address: PublicKey): Promise<RiskOutputRegister | null> {
  ...
  static async fetchMultiple(c: Connection, addresses: PublicKey[]): Promise<Array<RiskOutputRegister | null>> {
