...
const Overlay: FC<{
  ...
  const { getAskSymbolFromPair, setSelectedMarket } = useMarket()

  const handleClick = useCallback((pair: string) => {
    setArrowRotation(false)
    setSelectedMarket({ decimals: 3, market: 'crypto', pair })
    setVisible(false)
  }, [setArrowRotation, setSelectedMarket, setVisible])

  const markets = useMemo(() => (
    MARKETS
      .filter(({ name }) => SUPPORTED_TOKEN_LIST.find((token) => getAskSymbolFromPair(name) === token))
      .map(({ name }, index) => (
